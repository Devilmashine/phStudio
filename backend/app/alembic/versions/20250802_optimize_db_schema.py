...
def upgrade():
    ...
    op.create_index('idx_calendar_events_date_range', 'calendar_events',
                    ['start_time', 'end_time'])
    op.create_index('idx_bookings_date_range', 'bookings',
                    ['start_time', 'end_time'])
    
    # 2. Добавляем внешний ключ для связи booking и calendar_event
    op.add_column('bookings', sa.Column('calendar_event_id', 
                 sa.Integer, sa.ForeignKey('calendar_events.id')))
    
    # 3. Добавляем поле для кэширования состояния доступности
    op.add_column('calendar_events', sa.Column('availability_cached', 
                 sa.String(20), nullable=True))
    op.add_column('calendar_events', sa.Column('cache_updated_at', 
                 sa.DateTime, nullable=True))
    
    # 4. Добавляем поле для оптимизации поиска по номеру телефона
    op.add_column('bookings', sa.Column('phone_normalized', 
                 sa.String(20), nullable=True))
