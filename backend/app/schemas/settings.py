...
class StudioSettingsBase(BaseModel):
    ...
    work_days: List[str] = Field(
        ..., description="Список рабочих дней недели (например, ['mon', 'tue', ...])"
    )
    ...
    weekend_price_multiplier: float = Field(
        1.5, description="Множитель цены для выходных"
    )
    ...
    holidays: Optional[List[str]] = Field(
        default=None, description="Список праздничных дней (YYYY-MM-DD)"
    )
    min_booking_duration: int = Field(
        1, description="Минимальная длительность бронирования (часы)"
    )
    max_booking_duration: int = Field(
        8, description="Максимальная длительность бронирования (часы)"
    )
    advance_booking_days: int = Field(
        30, description="За сколько дней вперёд можно бронировать"
    )
